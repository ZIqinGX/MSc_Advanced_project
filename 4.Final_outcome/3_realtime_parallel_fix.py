...
def create_note_by_midi(m, dur=1):
    ...
    n.duration.type = 'quarter'  # Make it a half note (default is quarter)
...
def capture_and_render(note_info):
    global note_done, running
            ...
            for handlms in results.multi_hand_landmarks:
                for index, lm in enumerate(handlms.landmark):
                    if index == 8:  
                        ...
                        #     print('note indfo queue size is', note_info.qsize())
                            # ==========================
...
def play_music(note_info):
    global note_done, running
    while running:

        # print("note info, done info", note_info.qsize())
        try:
            if (not note_info.empty()):
                ...
                note_done = True
                try:
                    note_info.queue.clear()
                except:
                    print('couldnt clear note info')
                
                print ('gets to end of note info')
...
if ( __name__ == "__main__"):
    ...
    # finl note -- the processed note after duration info
    # done_info = queue.Queue()
    # done_info.put(True)
    note_done = True
    t1 = threading.Thread(target=capture_and_render, args=(note_info, ))
    t2 = threading.Thread(target=play_music, args=(note_info, ))
